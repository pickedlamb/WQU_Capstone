...
def read_data(filename,token_name,from_date, to_date,interval,l,lookback,holdingdays):
...
longterm['weekly']={'lookback':[5,10,20,60,120],'holdingdays':5,'basket_elements':[1]}
...
# longterm['quarterly']={'lookback':[20],'holdingdays':60,'basket_elements':[1]}
# longterm['halfyearly']={'lookback':[60],'holdingdays':120,'basket_elements':[1]}
# longterm['yearly']={'lookback':[120],'holdingdays':240,'basket_elements':[1]}
	...
	for basket_elements in longterm[strategy]['basket_elements']:
		...
		for l in indexwise_dict.keys():
			for lookback in longterm[strategy]['lookback']:
				...
				df_final = read_data('/Users/akshaykulkarni/Desktop/WQU/indexrawmedium/'+str(l)+'_rawmediumindex.xlsx',indexwise_dict[l], from_date, to_date,interval,l,lookback,holdingdays)
				print('data read')
				#df_final.to_excel('test_df.xlsx')
				basket_df=createbasket(df_final,basket_elements,True,lookback,holdingdays)
				print('baskets generated')
				return_df=getbasketreturn(df_final,basket_df,holdingdays,basket_elements)
				print('got the returns')
				# return_df.to_excel('test_retdf.xlsx')
				output_df[l+','+str(lookback)+' days,index']=return_df['afterholding'].apply(lambda x: (x)/100 +1 )
				# output_df.to_excel('test.xlsx')
				# exit()	
				
		output_df.to_excel(str(strategy)+'_'+str(basket_elements)+'_elements_medium_indexeqwt'+'.xlsx')
